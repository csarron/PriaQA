...
//	protected static final String WIKIPEDIA = "wikipedia";
     ...
     * @return a HashMap mapping the terms in the web serach results to their frequency in the
     * snippets
     */
     ...
     * @return an array of strings containing the variations of the target String, including the
     * original target
     */
    ...
    private void extractUpperCaseParts(ArrayList<String> targets) {
        ...
        for (Iterator<String> iter = duplicateFreeTargets.iterator(); iter.hasNext(); ) {
            ...
            while (i < targetTokens.length) {
                ...
                while ((i < targetTokens.length) && !Character.isUpperCase(targetTokens[i].charAt(0)))
                    i++;
...
//		lastTargetTermCounters = termCounters;
    ...
    /**
     * add all the term counters in source to target (perform a union of the key sets, summing up
     * the counters)
     */
