...
public class StringUtils {
    ...
    public static boolean equalsIntersect(String s1, String s2) {
        ...
        for (String token1 : tokens1)
            for (String token2 : tokens2)
                if (token1.equals(token2)) commonTokens++;
                else totalTokens++;
    ...
    /**
     * Compares two strings, using the same criterion as the <code>equalsCommonNorm()</code> method,
     * but considers only words starting with a capital letter (proper nouns)
     *
     * @param s1 string 1
     * @param s2 string 2
