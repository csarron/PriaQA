...
/**
 * <p>A <code>Predicate</code> represents a predicate-argument structure as defined in the PropBank
 * project.<p>
 *
...
public class Predicate implements Comparable<Predicate>, Serializable {
    ...
    /**
     * Creates a predicate data structure from an annotated sentence and sets the verb and argument
     * terms.
     *
...
//			MsgPrinter.printErrorMsg("Annotation: " + annotated);
    ...
    public boolean dropArgs(String s) {
        ...
        for (int i = 0; i < args.length; i++)
            if (args[i] != null && args[i].contains(s)) {
                args[i] = null;
                dropped = true;
     ...
     * @return a negative integer, zero or a positive integer as this predicate is less than, equal
     * to or greater than the specified predicate
     */
     ...
     * @return verb or argument, or <code>null</code> if the identifier is invalid or the argument
     * does not exist
     */
    ...
    /**
     * Returns all arguments that are not <code>null</code>, including missing arguments, in an
     * array.
     *
     * @return array containing the arguments
     */
    ...
    /**
     * Returns all semantic roles that are not <code>null</code>, including missing roles, in an
     * array.
     *
     * @return array containing the semantic roles
     */
    public String[] getRolesMissing() {
        ...
        for (int i = 0; i < missingArgs.length; i++)
            rolesMissing[roles.length + i] = missingArgs[i];
    ...
    private void setArgTerms(Term[] terms) {
        ...
        for (String arg : args)
            if (arg != null) tokenizedArgs.add(NETagger.tokenizeWithSpaces(arg));
